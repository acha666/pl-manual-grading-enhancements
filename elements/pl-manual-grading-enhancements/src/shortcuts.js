(() => {
  ...
  PLMGE.ShortcutManager = class ShortcutManager {
    constructor(contract, rubric, isEnabled) {
    ...
    sync() {
      ...
      const reservedKeys = new Set(
        this.contract.items
          .filter((item) => !item.groupName && PLMGE.DIGIT_KEYS.includes(item.originalKey))
          .map((item) => item.originalKey),
      );
      const availableKeys = PLMGE.DIGIT_KEYS.filter((key) => !reservedKeys.has(key));
    ...
    handleKeypress(event) {
      if (!PLMGE.DIGIT_KEYS.includes(event.key)) return;

      if (event.target instanceof Element && event.target.closest(".plmge-options-menu")) {
